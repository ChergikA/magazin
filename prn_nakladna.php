<?php
// накладную выписываем пока только для частника
 /*  	include("magazin.inc");
    $f = new Work_files();
	$consts = new Consts();
	$firm = $consts->firma;
	...
	$nalog_data = '&nbsp;&nbsp;ІПН : &nbsp;' . $consts->inn . ',&nbsp;&nbsp; номер свідоцтва:'  . $consts->nom_svidotstva;
	$nom_chek = $_GET['nom_chek'];
	$name_klient = $_GET['name_klient'];

    $filename =  $_GET['file'] ;  // печать с перечня счетов
	if($filename == '' ) $filename = $f->f_name; // печать с открытого счета
*/
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1069 style='border-collapse:
 ...
 <tr height=23 style='mso-height-source:userset;height:17.25pt'>
  ...
  <td colspan=7 class=xl41><?php echo 'НАКЛАДНА №_' .  $nom_chek . '&nbsp;&nbsp;від:&nbsp;'. $dateDoc .'&nbsp;p.' ?></td>
 ...
 <tr height=25 style='mso-height-source:userset;height:18.75pt'>
  ...
  <td class=xl21>За довiренiстю:</td>
  <td colspan=6 class=xl45>Серiя:<span
  style='mso-spacerun:yes'>                  </span>№_<span
  style='mso-spacerun:yes'>           </span>вiд:<span
 ...
 <tr class=xl27 height=17 style='height:12.75pt'>
  ...
  .5pt solid black'>Найменування товару</td>
  <td class=xl22 style='border-top:none;border-left:none'>Од.</td>
  <td class=xl22 style='border-top:none;border-left:none'>К-ть</td>
  <td class=xl26 width=48 style='border-top:none;border-left:none;width:36pt'>Ціна</td>
  <td class=xl26 width=70 style='border-top:none;border-left:none;width:53pt'>Сума</td>
  <td class=xl20></td>
  ...
  <td class=xl22 style='border-top:none;border-left:none'>Од.</td>
  <td class=xl22 style='border-top:none;border-left:none'>К-ть</td>
  <td class=xl26 width=45 style='border-top:none;border-left:none;width:34pt'>Ціна</td>
  <td class=xl26 width=71 style='border-top:none;border-left:none;width:53pt'>Сума</td>
 </tr>


  <?php
 // выводим данные продаж в таблицу
 /* если файл существует и доступен для записи.
	if (is_writable($filename)) {

	// код, товар, цена, остаток, к-во ,скидка, номерДок, Юзер, время, допКол, видДок(строки)
	...
	$nom_str = 1; $sum_itog=0;  $sum_sum=0; $skidka=0;
		while (!feof($file_list)) {
	        ...
	        if ($srt_arr[0] == '') continue; //последняя строка

	       	//$pos = stripos($srt_arr[10],'Чек');
			//if ($pos === false ) continue;  /* echo "Искомая строка NOT встречена в позиции номер $pos ";
	        if ($srt_arr[6] != $nom_chek ) continue;
	        if ( trim($srt_arr[10]) != 'Счет' ) continue;
...
<!--// <tr height=17 style='height:12.75pt'>
  ...
  <td colspan=7 class=xl43>Сума вказана з урахуванням знижки - 39%</td>
  <td class=xl19></td>
 ...
 <tr height=17 style='height:12.75pt'>
  <td height=17 class=xl19 style='height:12.75pt'></td>
  <td class=xl34>Відвантажив(ла)</td>
  <td class=xl35>&nbsp;</td>
  <td class=xl39></td>
  <td class=xl36>&nbsp;</td>
  <td class=xl34>Отримав(ла)</td>
  <td class=xl36>&nbsp;</td>
  <td class=xl36>&nbsp;</td>
  <td colspan=2 class=xl19 style='mso-ignore:colspan'></td>
  <td class=xl34>Відвантажив(ла)</td>
